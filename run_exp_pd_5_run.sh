...
do
  ...
  awk '/Accuracy|Sensitivity|Specificity|PPV|NPV/{print $0}' ${output_file} | awk '
  BEGIN{
    count=0;
    sum_acc=sum_sen=sum_spe=sum_ppv=sum_npv=0;
    sq_sum_acc=sq_sum_sen=sq_sum_spe=sq_sum_ppv=sq_sum_npv=0;
  }
